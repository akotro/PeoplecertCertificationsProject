...
export default function CandidateEdit(props) {
  ...
  const handleSubmit = (event) => {
    ...
    if (params.id === undefined) {
      ...
      axios
        .post(`https://localhost:7196/api/Candidate`, candidate)
      ...
      axios
        .put(`https://localhost:7196/api/Candidate/${params.id}`, candidate)
        .then(function (response) {
          console.log(response);
        })
        .catch(function (error) {
  ...
  const handleChange = (event, addressIndex) => {
    ...
    if (addressIndex === undefined) {
      if (name === "gender") {
        setCandidate({
          ...candidate,
          [name]: genders.find((item) => item.id === Number(value)),
        });
      } else if (name === "language") {
        setCandidate({
          ...candidate,
          [name]: languages.find((item) => item.id === Number(value)),
        });
      } else if (name === "photoIdType") {
        setCandidate({
          ...candidate,
          [name]: photoIdTypes.find((item) => item.id === Number(value)),
        });
      } else if (type === "date") {
      ...
      setCandidate({
        ...candidate,
        address: candidate.address.map((address, index) => {
          if (index === addressIndex) {
            if (name === "country") {
              return {
                ...address,
                [name]: countries.find((item) => item.id === Number(value)),
              };
            } else {
              return {
                ...address,
  ...
  return (
    <div>
                    ...
                    <div key={index} name={item.id} className="my-1 ">
                      <Row>
                        <details className="display-6 fs-4">
                          <summary>Address {index + 1}</summary>
      ...
      <Button
        variant="dark"
        className="d-grid gap-2 col-12 mx-auto py-2 my-2"
        onClick={() => navigate(-1)}
      >
        Go back
      </Button>
